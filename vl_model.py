...
class VLBertModel(nn.Module):
    def __init__(
        self, num_labels, text_pretrained="bert-base-uncased", geoloc_features=14
    ):
        super().__init__()
...
class AlbefModel(nn.Module):
    ...
    def load_albef_pretrained(num_out_labels, geoloc_features=14):
        ...
        if not os.path.exists(albef_bert_config_fp):
            urlretrieve(
                "https://raw.githubusercontent.com/salesforce/ALBEF/main/configs/config_bert.json",
                albef_bert_config_fp,
            )
